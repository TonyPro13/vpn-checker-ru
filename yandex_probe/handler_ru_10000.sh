...
BASE_WORK="/tmp/mihomo-ru10000-${REQUEST_ID:-manual}"
...
SUMMARY="$(
  jq -cn \
    ...
    | {
        ...
        alive:($alive_nodes|length),
        dead_or_timeout:(($meta.converted // 0)-($alive_nodes|length)),
        latency_ms:{
