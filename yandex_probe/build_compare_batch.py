...
def main():
    ...
    if args.limit <= 0 or args.limit >= len(all_uris):
        ...
        # Deterministic spread across the entire pool.
        # This avoids taking only one clustered protocol/source segment.
        n = len(all_uris)
        chosen_indexes = []
        seen = set()
        ...
        # Defensive fill if integer rounding ever produced fewer entries.
    ...
    uris = [uri for _, uri in selected_pairs]
    ...
    meta = {
        ...
        "sample_source_indexes": [idx for idx, _ in selected_pairs],
        "selected": len(uris),
